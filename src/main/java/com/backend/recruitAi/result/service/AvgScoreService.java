...
public class AvgScoreService {
    ...
    public AvgScoreDto calculateAverageScores(List<InterviewResult> results) {
        if (results == null || results.isEmpty()) {
            return new AvgScoreDto(0, 0, 0, 0, 0, 0);
        }
        ...
        return new AvgScoreDto(
                (int) (totalScore / resultCount),
                (int) (totalEmotionScore / resultCount),
                (int) (totalBlinkScore / resultCount),
                (int) (totalEyeScore / resultCount),
                (int) (totalHeadScore / resultCount),
                (int) (totalHandScore / resultCount)
        );
    }
}